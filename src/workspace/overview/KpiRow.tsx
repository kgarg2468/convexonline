...
/**
 * One KPI tile: label → value → delta row → optional sparkline (one visual per
 * tile). The value is a string so the tile never formats anything itself.
 */
function KpiTile({
  label,
  value,
    ...
    <li className={tileClass}>
      ...
      {series ? (
        <>
          ...
          <span className="sr-only">{seriesSentence}</span>
        </>
      ) : null}
    </li>
  );
...
/**
 * The four KPIs (design-spec §4.1). Only the replies tile has a day series to
 * draw (the backend keeps one 14-day series); the other tiles' visual is the
 * delta row.
 */
export function KpiRow({ kpis, series }: { kpis: OverviewSummary["kpis"]; series: OverviewSummary["series"] }) {
  ...
  const perDay = series.repliesPerDay14.map((d) => d.count);
  return (
    <ul aria-label="Key figures" className="grid gap-3 @md:grid-cols-2 @3xl:grid-cols-4">
      <KpiTile
        ...
        series={perDay}
        seriesSentence={`Replies per day over the last 14 days, oldest first: ${perDay.join(", ")}.`}
      />
      <KpiTile

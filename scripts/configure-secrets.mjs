 ...
 *   node scripts/configure-secrets.mjs --deployment X        # named deployment
 *
 ...
 *
 * Scope: only the provider API keys listed in REQUIRED_KEYS. The AgentMail
 * webhook signing secret (AGENTMAIL_WEBHOOK_SECRET) is NOT handled here: it is
 * generated by AgentMail when the product-scoped webhook is registered, and the
 * separate register-webhook setup owns the hook identity and stores that secret
 * on the deployment. This script never reads, requires or overwrites it.
 *
 * Safety properties:
...
// Provider API keys only. AGENTMAIL_WEBHOOK_SECRET is deliberately absent: the
// register-webhook setup provisions the hook and its signing secret.
const REQUIRED_KEYS = ["OPENAI_API_KEY", "FIRECRAWL_API_KEY", "AGENTMAIL_API_KEY"];
...
function main() {
  ...
  console.log(`configure-secrets: ${REQUIRED_KEYS.length} provider keys set`);
  console.log(
    "configure-secrets: AGENTMAIL_WEBHOOK_SECRET is not managed here; run the register-webhook setup to provision the hook and its signing secret.",
  );
}

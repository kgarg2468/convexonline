...
### 2026-09-22 - 549ed9c
Started the interface revamp with a styling foundation that changes tooling before any view. Tailwind v4 and shadcn radix-nova primitives are installed behind a single CSS entry that layers the legacy stylesheet beneath utilities, so later views can replace rules without specificity fights. Design tokens are OKLCH custom properties: a teal-green accent, a secondary hue rotated 137.5 degrees from it, three ink levels checked for at least 4.5:1 contrast, semantic tints, rail colours, motion durations and easings. Fonts are self-hosted Geist, Geist Mono and Newsreader.
Automated review found four issues; all were fixed: generated dialog, sheet, popover and menu animations now collapse to a short cross-fade under reduced motion, the command palette's hidden title sits inside the dialog subtree, the popover title renders a real heading, and the MCP entry pins the audited package version. A stray machine-specific symlink was removed. All 386 offline tests, lint, types and build pass; 27 local browser scenarios pass with two optional captures skipped. Deployed on Convex static hosting, which serves the app for deep paths so pathname routing can follow.
Evidence: `src/styles/app.css`, `src/App.tsx`, `src/components/ui/`, `src/workspace/workspace.css`, `PRODUCT.md`.

### 2026-09-22 - b480018
Added the backend for an Overview dashboard: one query returns needs-action counts, the five most urgent threads, seven-day KPIs with previous-window comparisons, a fourteen-day replies series in the inn's local days, draft-outcome and inquiry breakdowns, and three side-by-side comparisons (this week against last, before and after the latest page change, drafted against sent text). Reads use the stats aggregates once they are ready and bounded index scans otherwise.
Automated review found scans whose caps could drop the rows that matter: corrections were read in status order rather than recency, and reminder rows were read oldest-first. Corrections are now walked newest-first per status, and a new follow-ups index by thread and status answers the due-reminder question directly. Six tests cover the empty inn, seeded windows, ordering, edit flags, cross-inn access and agreement between the aggregate and scanned paths; all 392 offline tests pass. Deployed on Convex with the new index.
Evidence: `convex/overview.ts`, `convex/schema.ts`, `tests/overview.test.ts`.

### 2026-09-22 - d5a1b33
Rebuilt the application shell. The workspace now lives at pathname routes (inbox, thread, policy changes, knowledge, settings), so reloads, deep links and the browser back button restore the view and the open thread; the invitation fragment is never rewritten. The rail is rebuilt with icons, live counts, a remembered collapsed state and a visible active bar; below 900px the same navigation becomes a bottom tab bar. A command palette jumps to views, opens threads and runs the demo policy action, with g-chords, a shortcuts sheet and Escape. View swaps use React view transitions with a short cross-fade on desktop and a drawer slide on phones, collapsing to opacity under reduced motion.
Automated review and a separate taste review found three correctness issues and a dozen craft issues; all were fixed: a deep-linked thread from another property no longer renders inside the wrong workspace, going back to the list retraces history instead of growing it, a failed palette action stays visible, the active bar and focus ring now contrast on the dark rail, the header is a fixed height with stats in a strip beneath on phones, and focus moves to the main region on navigation with a polite announcement. All 402 offline tests, lint, types and build pass; all 29 browser scenarios pass on desktop and phone. Deployed on Convex.
Evidence: `src/workspace/lib/router.ts`, `src/workspace/shell/`, `src/styles/motion.css`, `tests/router.test.ts`.

### 2026-09-22 - d8eb059
Rebuilt the inbox frame. The view is a three-column grid: a queue of guest threads with search, a status filter and a skeleton on first load; the open thread; and, on wide screens, the draft's sources. Between 900 and 1200 pixels the sources become a header sheet, and on phones the list and the thread are separate screens. The header carries the day's numbers: replies today in the inn's time zone, median first response, and open threads split into "need you" and "ready". Keyboard users move with j and k and open with Enter, and keyboard selection never animates the selection bar.
Automated review and a separate taste review found a stale empty list posing as "no matches" for a new search, duplicate ids between the sources pane and its sheet, header actions overlapping the title between 900 and 1200 pixels so the sources button could not be clicked, filter and scroll position lost on the phone round trip, and a kept thread that stayed interactive while the next one loaded. All were fixed: the empty state waits for the new query, the sheet has its own title id, the header actions no longer shrink, the filter and scroll survive list-to-thread-and-back, and the previous thread is inert and dimmed until the new one arrives. New browser coverage runs at 1000 by 800 for the sources sheet and exercises j, k, Enter and the send shortcut. All 402 offline tests, lint, types and build pass; all 30 browser scenarios pass on desktop, mid-width and phone. Deployed on Convex.
Evidence: `src/workspace/inbox/InboxView.tsx`, `src/workspace/inbox/QueueList.tsx`, `src/workspace/inbox/InboxStats.tsx`, `tests/browser/specs/inbox-mid.spec.ts`.

### 2026-09-22 - ef53b10
Rebuilt the thread pane. The open thread has a sticky header with the subject, guest and status, stay facts with icons, a claim bar, a presence line, the guest's message in serif italic and the reply cards. The draft panel sends on Cmd or Ctrl plus Enter and its box glides into place as the sent message. Every sentence of the draft that matches a claim is underlined; hovering or focusing it highlights the source card that the claim rests on, and hovering the card highlights the sentence back. After a send the draft folds to a one-line summary with the delivery state, and the follow-up email is a collapsible section that opens only when an approval is offered or a reminder is scheduled or failed. The demo's simulated inquiry form was moved onto the same primitives, fixing a select that overflowed its column.
A taste review found the missing citation link, the always-open follow-up card, the doubled sent text, an unreachable foreign-thread state and a translucent sticky header; all were fixed. Automated review then found that paraphrased claims lost their citations, so the matcher gained a second pass that scores sentences by content-word overlap with numbers required to agree, and a further finding that negation was discarded, so "not allowed" now marks its word and contradicting pairs are refused. The security suite found that a refused thread deep link blanked the whole app; the inbox frame now reads the subscription without throwing and shows "This thread could not be opened" with a way back while the queue stays up, and the demo workspace sits under the same access boundary as a real property. Twelve unit tests cover the claim matcher. All 414 offline tests, lint, types and build pass; all 31 browser scenarios pass on desktop, mid-width and phone. Deployed on Convex.
Evidence: `src/workspace/inbox/ThreadDetail.tsx`, `src/workspace/inbox/DraftPanel.tsx`, `src/workspace/inbox/CitedText.tsx`, `src/workspace/inbox/claimMatch.ts`, `src/workspace/lib/hooks.ts`, `tests/claim-match.test.ts`.

### 2026-09-22 - bc34033
Rebuilt the policy changes view. The header states how many replies need review, how many were sent unchanged and how many were edited, and each change card shows the old and new passage as a word-level diff: split side by side from 901 pixels and unified below, with markdown markers stripped so the reader sees prose. Cards open and close through a disclosure that names what it controls, and the demo actions sit on the shared button. A cross-review found two blockers and four should-fix items, all applied: the stats grid collapsed to a single line under 901 pixels, the entrance animation is gated on a settled flag so the first paint never animates, and the disclosure announces its state. All offline tests, lint, types and build pass; all 31 browser scenarios pass. Deployed on Convex.
Evidence: `src/workspace/corrections/ChangesStats.tsx`, `src/workspace/corrections/PassageDiff.tsx`, `src/workspace/corrections/wordDiff.ts`, `src/workspace/corrections/plainText.ts`, `tests/plain-text.test.ts`.

### 2026-09-22 - ce0d284
Rebuilt settings and sign-in. Settings is a set of sections with key-value lists and action rows; the team section labels roles, confirms removal in a popover that shows its own error, and lists invitations with a one-use link. Sign-in is a card with the wordmark as the page heading and two tabs, both mounted so the browser can autofill either. Errors on actions are announced as alerts; passive notices are not. A cross-review's blockers and should-fix items were all applied. All offline tests, lint, types and build pass; all 31 browser scenarios pass. Deployed on Convex.
Evidence: `src/workspace/settings/primitives.tsx`, `src/workspace/settings/TeamSettings.tsx`, `src/workspace/auth/AuthCard.tsx`, `src/workspace/auth/AuthView.tsx`.

### 2026-09-22 - f486ac9
Rebuilt the knowledge view as a page table. Each crawled page is a row with its title, latest version, watch switch and actions; the content opens in a side sheet with the version history; facts are added from a popover that keeps the table in view. The header meta line carries the page count, the watched count and the last crawl. A cross-review found a heading row whose columns drifted from the cells, dividers that stayed on phones, a popover that opened upward without a heading, a switch with no accessible name and a first paint that animated; all were fixed, with measured heading and cell positions matching at 1280 and 1000 pixels. All offline tests, lint, types and build pass; all 31 browser scenarios pass. Deployed on Convex.
Evidence: `src/workspace/knowledge/KnowledgeView.tsx`, `src/workspace/knowledge/PageRow.tsx`, `src/workspace/knowledge/ContentSheet.tsx`, `src/workspace/knowledge/AddFactPopover.tsx`, `src/workspace/knowledge/WatchSwitch.tsx`.

### 2026-09-22 - aabf8b1
Added the security suite. Eight browser scenarios type six script payloads into every real write path (simulated inquiry, gap answer, staff fact, property name, invitation label, website editor) and read them back as text from the rendered DOM, counting elements rather than source; hostile ids in the address bar and hostile invitation fragments are exercised the same way, and the public inn website is fetched over plain HTTP and opened in a browser. Fifteen unit tests call all 43 id-taking functions with malformed ids, wrong-table ids and another inn's real ids, and prove the crawl cooldown, the hourly run cap, the regenerate cooldown and the model budget refuse without side effects. The suite found that JSON responses lacked a nosniff header, fixed here, and that a refused thread deep link blanked the app, fixed on the thread branch. All offline tests, lint, types and build pass. Deployed on Convex.
Evidence: `tests/browser/specs/security.spec.ts`, `tests/security.test.ts`, `convex/http.ts`, `.context/uiux/reviews/security-findings.md`.

### 2026-09-22 - 02fcad8
Added the Overview view, the first destination for a real property. It opens with a needs-action strip of four tiles (needs you, ready to send, policy changes to review, follow-ups due) that deep-link into the inbox with the matching filter kept in the address, an up-next list of the most urgent threads, four seven-day key figures with previous-window deltas, a fourteen-day replies chart with a labelled peak, draft-outcome and inquiry-mix bars, and three comparisons: this week against last, before and after the latest page change, and drafted against sent text. The rail and the phone tab bar gain a fifth item, `g o` jumps to it, and the demo still lands on policy changes.
Automated review found a heading that implied replies when none were sent and a machine-specific test config swept into the branch; a separate taste review found sub-text below AA contrast on tinted tiles, a page-change comparison whose two columns held different rows and read as a halving, and a filter query dropped from the URL so reload and Forward lost the queue, plus empty KPI card space, a header repeating the strip, an unscaled day chart, unlabelled tile rows, a strip filling a phone's first screen and 10px tab labels. All were fixed. All 437 offline tests, lint, types and build pass; 44 browser scenarios pass on desktop, mid-width and phone, including Back and Forward keeping the inbox filter. Deployed on Convex.
Evidence: `src/workspace/overview/OverviewView.tsx`, `src/workspace/overview/NeedsActionStrip.tsx`, `src/workspace/lib/hooks.ts`, `tests/browser/specs/overview.spec.ts`.

### 2026-09-22 - 87fe7bb
Closed three findings from the independent browser QA round: Policy changes always shows its three tiles, including zero pages changed before the demo change; Knowledge leads with the same three-tile strip instead of header prose; Settings groups the website address and editor under a Public website heading. The fourth finding, auth tabs not activating on arrow keys, did not reproduce in about fifteen replays across viewports and CPU throttling, so the auth spec now asserts the behaviour instead. Lint, types, 437 offline tests and build pass; the browser suite passes. Deployed on Convex.
Evidence: `src/workspace/corrections/ChangesStats.tsx`, `src/workspace/knowledge/KnowledgeStats.tsx`, `src/workspace/settings/InnWebsiteEditor.tsx`, `tests/browser/specs/auth.spec.ts`.

### 2026-09-22 - b10de58
Made the inbox dense and calm after the thread view was judged clunky and awkwardly spaced. Modelled on Front, Gmail and Superhuman: queue rows are two lines at 63 pixels (name, claim holder, status pill and time; then subject and snippet on one line), the queue is 300 pixels, the sources pane appears only from 1440 pixels with the header's Sources sheet below that, so the thread pane grows from about 390 to 748 pixels at 1280. The thread head is one block with the claim and presence state folded in, messages are flat with hairline dividers, the gap form is one panel, and every inbox surface uses one type scale and one spacing scale.
Automated review found a staff claimant name with no width cap that could clip the timestamp; a separate taste review found the status pill stealing width from the subject on exactly the rows that need action, snippet and helper text below AA contrast on hover and on the warm panel, three different right edges in the thread stack between 1200 and 1439 pixels, a title that truncated at the wider viewport, 28-pixel controls on phones, and a sticky head taking a quarter of a phone screen. All were fixed. Lint, types and build pass; all 44 browser scenarios pass at 1440, 1000 and phone widths. Deployed on Convex.
Evidence: `src/workspace/inbox/QueueRow.tsx`, `src/workspace/inbox/ThreadDetail.tsx`, `src/workspace/inbox/InboxView.tsx`, `tests/browser/playwright.config.ts`.
